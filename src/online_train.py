...
def flatten_tree(node):
    """
    flatten the tree structure to a list, collect the parent node, child node and the corresponding value
    """
    parents = []
    children = []
    values = []
...
def cal_meta_transition_probs(node):
    ...
    # use numpy advanced indexing and accumulation to update the matrix
    if len(parents) > 0:
        np.add.at(TransitionProbs, (parents, children), values)
    return TransitionProbs

def np_softmax(x):
    # do softmax on each row of the matrix
...
def sampling_meta_action(node, num=1, TransitionProbs=None):
    ...
    # calculate the softmax of the transition probabilities
...
class TreeNode:
    ...
    def get_child_policy_prob(self, child):
        # extract logit values and convert to array
        logits = torch.tensor(list(self.policy.values()))
        prob, log_prob = robust_softmax(logits)

        # build a new dictionary that maps keys to Softmax probabilities
...
# length-normalized log-probabilities and entropy and variance of entropy
...
def forward_value_predict(model, tokenizer, inputs):
    ...
    # retrive the token id of "<positive_rating>" and "<negative_rating>"
    positive_token_id = tokenizer.convert_tokens_to_ids("<positive_rating>")
    negative_token_id = tokenizer.convert_tokens_to_ids("<negative_rating>")
    ...
    # expand indices to match the shape of logits
    selected_logit = logits[range(batch_size), pos]  # [batch_size, num_tokens]
    selected_logit = selected_logit[:, indices]      # extract the logits of the specified token in each row
...
def compute_gae_from_node(node, gamma=0.99, lambda_=0.95):
    ...
    # traverse the path from root to the specified node and compute GAE
    gae = 0
    factor = 1  # used to accumulate (gamma * lambda)

    # traverse the path from root to the specified node
    for i in range(len(path) - 1):  # path[-1] is the target node, we don't need to compute TD error
        current_node = path[i]
        next_node = path[i + 1]
        ...
        # accumulate TD error based on GAE
        gae += factor * td_error
        # update factor to prepare for the next step
...
class Environment:
    def __init__(self, problems):
        """
            ...
            Parameters:
            - problems: A list of dictionaries containing math problems and answers, where each dictionary has 'problem' and 'ground_truth' keys.
        """
    ...
    def sample_initial_state(self):
        """
        sample a math problem from the list of problems

        return:
        - initial_state: the problem text
    ...
    def is_terminal_state(self, state, ground_truth):
        """
        check if the state is the terminal state (ground truth)
        ...
        return:
        - is_terminal: boolean, whether the state is the terminal state

        """
        # use compute_rule_orm_head function to check
    ...
    # check if the state is the correct answer
    def compute_rule_orm_head(self, node):
        """
        use check function from grading module to check if the state is the correct answer
        ...
        return:
        - result: boolean, whether the state is the correct answer
...
# Assume you have defined TreeNode, MCTS and RLSPTrainer classes
...
lora_config = LoraConfig(
    r=32,  # rank of the low-rank matrix
    lora_alpha=16,  # scaling factor of LoRA
    target_modules=["k_proj","q_proj","o_proj", "v_proj","down_proj","gate_proj","up_proj",],  # target modules, generally query and key projection layers
    lora_dropout=0.1,  # dropout probability
    bias="none", # not including bias in LoRA
)

# use peft to convert the model to a LoRA model
...
# initial state and MCTS parameters
num_simulations = 16
num_candidates_per_expansion = 2
